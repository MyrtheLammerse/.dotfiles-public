...
prompt_terraform-workspace() {
  ...
  while [[ "$tfdir" != "/" ]]; do
    if [[ -d "$tfdir/.terraform" || -n "$tfdir"/*.tf(N) ]]; then
      found=true
      break
    fi
    tfdir="${tfdir:h}"
  ...
  local ws
  ws=$(terraform -chdir="$tfdir" workspace show 2>/dev/null) || return
  [[ -z "$ws" || "$ws" == "default" ]] && return

  if [[ "$ws" == *-* ]]; then
    local parts
    parts=(${(s/-/)ws})
    (( ${#parts} >= 3 )) && ws="${parts[-3]}-${parts[-2]}-${parts[-1]}"
  fi

  (( ${#ws} > 28 )) && ws="${ws[1,25]}..."

  prompt_segment yellow black "${ws:gs/%/%%}"
}
